...
class Picture(Element):
    ...
    def render(self, offset):
        ...
        self.drawing.transform.translate(
            0.5 * w, h - (0.5 * self.height + offset) * h * float(w) / float(h)
        )
...
class Credits(Layer, KeyListener):
    ...
    def __init__(self, engine, songName=None):
        ...
        # Avoid AttributeError in hidden()/run() paths before songLoaded() runs
        self.song = None

        self.songLoader = self.engine.resource.load(
            self,
            "song",
            lambda: Song.loadSong(self.engine, "defy", playbackOnly=True),
        ...
        self.credits = [
            ...
            Picture(self.engine, "logo.svg", 0.25),
            Text(nf, bs, c2, "center", " "),
            Text(nf, bs, c2, "center", _("Version %s") % Version.version()),
            # --- Added: Portuguese credit for the Python 3 port/modification ---
            Text(
                nf, bs, c1, "center", _("Versao modificada / portada em Python 3 por")
            ),
            Text(nf, bs, c2, "center", "Lucas Grassano Lattari"),
            # -------------------------------------------------------------------
            space,
    ...
    def keyPressed(self, key, unicode):
        if (
            self.engine.input.controls.getMapping(key)
            in [Player.CANCEL, Player.KEY1, Player.KEY2]
