...
class Guitar:
    ...
    def renderNeck(self, visibility, song, pos):
        ...
        glTexCoord2f(0.0, project(offset + l * beatsPerUnit * 0.7))
        glVertex3f(-w / 2, 0, l * 0.7)
        glTexCoord2f(1.0, project(offset + l * beatsPerUnit * 0.7))
        glVertex3f(w / 2, 0, l * 0.7)
    ...
    def renderBars(self, visibility, song, pos):
        ...
        while beat < currentBeat + self.beatsPerBoard:
            ...
            if z > self.boardLength * 0.8:
                c = (self.boardLength - z) / (self.boardLength * 0.2)
            elif z < 0:
                c = max(0, 1 + z)
            else:
                c = 1.0
            ...
            if (beat % 1.0) < 0.001:
                Theme.setBaseColor(visibility * c * 0.75)
            else:
                Theme.setBaseColor(visibility * c * 0.5)
        ...
        Theme.setSelectedColor(visibility * 0.5)
        glBegin(GL_TRIANGLE_STRIP)
        glTexCoord2f(0.0, 0.0)
        glVertex3f(-w / 2, 0, sw)
        glTexCoord2f(0.0, 1.0)
    ...
    def renderNote(self, length, color, flat=False, tailOnly=False, isTappable=False):
        ...
        glColor4f(0.75 * color[0], 0.75 * color[1], 0.75 * color[2], color[3])
        self.noteMesh.render("Mesh")
        glColor4f(0.25 * color[0], 0.25 * color[1], 0.25 * color[2], color[3])
    ...
    def renderNotes(self, visibility, song, pos):
        ...
        for time, event in track.getEvents(
            ...
            if z < 0:
                if event.played:
                    ...
                    color = (0.2 + 0.4, 0.2 + 0.4, 0.2 + 0.4, 0.5 * visibility * f)
                    flat = True

            glPushMatrix()
            glTranslatef(x, (1.0 - visibility) ** (event.number + 1), z)
            self.renderNote(
                length, color=color, flat=flat, tailOnly=tailOnly, isTappable=isTappable
            )
            glPopMatrix()
        ...
        for time, event in self.playedNotes:
            ...
            while t > time and t - step > pos and i < len(vertices) / 8:
                ...
                colors[i] = colors[i + 1] = (c[0], c[1], c[2], 0.5)
                colors[i + 2] = colors[i + 3] = (1, 1, 1, 0.75)
                colors[i + 4] = colors[i + 5] = colors[i + 6] = colors[i + 7] = (
                    c[0],
                    c[1],
                    c[2],
                    0.5,
    ...
    def renderFrets(self, visibility, song, controls):
        ...
        for n in range(self.strings):
            ...
            if f and (
                controls.getState(Player.ACTION1) or controls.getState(Player.ACTION2)
            ):
                f += 0.25
            ...
            if f:
                ...
                glPushMatrix()
                glTranslate(x, y, 0)
                glRotate(f + self.time * 0.1, 0, 1, 0)
                size = (0.22 * (f + 1.5), 0.22 * (f + 1.5))
    ...
    def getMissedNotes(self, song, pos):
        ...
        notes = [
            (time, event)
            for time, event in track.getEvents(pos - m1, pos - m2)
            if isinstance(event, Note)
        ]
        notes = [
            (time, event)
            for time, event in notes
            if (time >= (pos - m2)) and (time <= (pos - m1))
        ]
    ...
    def getRequiredNotes(self, song, pos):
        ...
        notes = [
            (time, event)
            for time, event in notes
            if (time >= (pos - self.lateMargin)) and (time <= (pos + self.earlyMargin))
        ]
        if notes:
    ...
    def run(self, ticks, pos, controls):
        ...
        if self.editorMode:
            if controls.getState(Player.ACTION1) or controls.getState(Player.ACTION2):
                activeFrets = [
                    i for i, k in enumerate(KEYS) if controls.getState(k)
                ] or [self.selectedString]
            else:
                activeFrets = []
        else:
        ...
        for n in range(self.strings):
            if controls.getState(KEYS[n]) or (
                self.editorMode and self.selectedString == n
            ):
