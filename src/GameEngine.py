...
Config.define("opengl", "svgshaders", bool, False)
...
Config.define(
    ...
    int,
    4,
    text=_("Antialiasing Quality"),
    options={0: _("None"), 2: _("2x"), 4: _("4x"), 6: _("6x"), 8: _("8x")},
)
...
Config.define(
    ...
    options=dict([(n, n) for n in range(1, 120)]),
)

# In the original file this was commented out, but the engine *uses* it.
# Keeping it enabled prevents None/KeyError issues when setting SVG quality.
Config.define(
    "opengl",
    "svgquality",
    int,
...
Config.define(
    ...
    int,
    44100,
    text=_("Sample Frequency"),
    options=[8000, 11025, 22050, 32000, 44100, 48000],
)
...
Config.define(
    ...
    100,
    text=_("A/V delay"),
    options=dict([(n, n) for n in range(0, 301)]),
)
Config.define(
    "audio",
    "screwupvol",
    float,
    0.25,
    text=_("Screw Up Sounds"),
    options={0.0: _("Off"), 0.25: _("Quiet"), 0.5: _("Loud"), 1.0: _("Painful")},
)
Config.define(
    "audio",
    "guitarvol",
    float,
    1.0,
    text=_("Guitar Volume"),
    options=dict([(n / 100.0, "%02d/10" % (n / 9)) for n in range(0, 110, 10)]),
)
Config.define(
    "audio",
    "songvol",
...
Config.define(
    ...
    1.0,
    text=_("Text scale"),
    options=dict([(n / 100.0, "%3d%%" % n) for n in range(50, 260, 10)]),
)
...
class GameEngine(Engine):
    ...
    def __init__(self, config=None):
        ...
        self.audio.pre_open(
            frequency=frequency, bits=bits, stereo=stereo, bufferSize=bufferSize
        )
        pygame.init()
        self.audio.open(
            frequency=frequency, bits=bits, stereo=stereo, bufferSize=bufferSize
        ...
        self.video.setMode(
            (width, height), fullscreen=fullscreen, multisamples=multisamples
        )
        ...
        if self.config.get("game", "uploadurl").startswith("http://kempele.fi"):
            self.config.set(
                "game", "uploadurl", "http://fretsonfire.sourceforge.net/play"
            )
    ...
    def toggleFullscreen(self):
        ...
        if not self.video.toggleFullscreen():
            # on windows, the fullscreen toggle kills our textures, so we must restart the whole game
