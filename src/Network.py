...
class Connection(asyncore.dispatcher):
    ...
    def connect(self, host, port=PORT):
        ...
        # do a blocking connect (original behavior)
        n = 0
    ...
    def handle_read(self):
        try:
            ...
            if data:
                ...
                if not self._receivedSizeField:
                    payload = self._packet.getvalue()

                    # The first packet contains the ID
                    if self.id is None:
                        self.id = struct.unpack("H", payload)[0]
                        self.handleRegistration()
                    else:
    ...
    def sendPacket(self, packet):
        # packets are expected to be bytes
...
class Server(asyncore.dispatcher):
    ...
    def broadcastPacket(self, packet, ignore=None, meToo=True):
        if ignore is None:
            ignore = []
        ...
        if meToo and self.clients:
            # In Py2 they did self.clients.values()[0] (first client).
