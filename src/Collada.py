...
def GetEuler(rotateElement):
    euler = [
        float(rotateElement[0]) * float(rotateElement[3]) * angleToRadian,
...
def __ToXml(xmlNode, indent="\t", newl="\n", totalIndent=""):
    ...
    if len(childs) > 0:
        ...
        for child in childs:
            ...
            result += "%s%s" % (
                tempnewl,
                __ToXml(child, indent, newl, totalIndent + indent),
            )
...
class DaeDocument(object):
    ...
    def __init__(self, debugM=False):
        ...
        self.animationsLibrary = DaeLibrary(
            DaeSyntax.LIBRARY_ANIMATIONS, DaeAnimation, DaeSyntax.ANIMATION
        )
        self.animationClipsLibrary = DaeLibrary(
            DaeSyntax.LIBRARY_ANIMATION_CLIPS,
            DaeAnimationClip,
            DaeSyntax.ANIMATION_CLIP,
        )
        self.camerasLibrary = DaeLibrary(
            DaeSyntax.LIBRARY_CAMERAS, DaeCamera, DaeSyntax.CAMERA
        )
        self.controllersLibrary = DaeLibrary(
            DaeSyntax.LIBRARY_CONTROLLERS, DaeController, DaeSyntax.CONTROLLER
        )
        self.effectsLibrary = DaeLibrary(
            DaeSyntax.LIBRARY_EFFECTS, DaeFxEffect, DaeFxSyntax.EFFECT
        )
        self.geometriesLibrary = DaeLibrary(
            DaeSyntax.LIBRARY_GEOMETRIES, DaeGeometry, DaeSyntax.GEOMETRY
        )
        self.imagesLibrary = DaeLibrary(
            DaeSyntax.LIBRARY_IMAGES, DaeImage, DaeSyntax.IMAGE
        )
        self.lightsLibrary = DaeLibrary(
            DaeSyntax.LIBRARY_LIGHTS, DaeLight, DaeSyntax.LIGHT
        )
        self.materialsLibrary = DaeLibrary(
            DaeSyntax.LIBRARY_MATERIALS, DaeFxMaterial, DaeFxSyntax.MATERIAL
        )
        self.nodesLibrary = DaeLibrary(DaeSyntax.LIBRARY_NODES, DaeNode, DaeSyntax.NODE)
        self.visualScenesLibrary = DaeLibrary(
            DaeSyntax.LIBRARY_VISUAL_SCENES, DaeVisualScene, DaeSyntax.VISUAL_SCENE
        )

        # Physics Support
        self.physicsMaterialsLibrary = DaeLibrary(
            DaeSyntax.LIBRARY_PHYSICS_MATERIALS,
            DaePhysicsMaterial,
            DaePhysicsSyntax.PHYSICS_MATERIAL,
        )
        self.physicsScenesLibrary = DaeLibrary(
            DaeSyntax.LIBRARY_PHYSICS_SCENES,
            DaePhysicsScene,
            DaePhysicsSyntax.PHYSICS_SCENE,
        )

        self.physicsModelsLibrary = DaeLibrary(
            DaeSyntax.LIBRARY_PHYSICS_MODELS,
            DaePhysicsModel,
            DaePhysicsSyntax.PHYSICS_MODEL,
        )
    ...
    def LoadDocumentFromFile(self, filename):
        ...
        self.imagesLibrary.LoadFromXml(
            self, FindElementByTagName(colladaNode, DaeSyntax.LIBRARY_IMAGES)
        )
        self.animationsLibrary.LoadFromXml(
            self, FindElementByTagName(colladaNode, DaeSyntax.LIBRARY_ANIMATIONS)
        )
        self.animationClipsLibrary.LoadFromXml(
            self, FindElementByTagName(colladaNode, DaeSyntax.LIBRARY_ANIMATION_CLIPS)
        )
        self.camerasLibrary.LoadFromXml(
            self, FindElementByTagName(colladaNode, DaeSyntax.LIBRARY_CAMERAS)
        )
        self.controllersLibrary.LoadFromXml(
            self, FindElementByTagName(colladaNode, DaeSyntax.LIBRARY_CONTROLLERS)
        )
        self.effectsLibrary.LoadFromXml(
            self, FindElementByTagName(colladaNode, DaeSyntax.LIBRARY_EFFECTS)
        )
        self.geometriesLibrary.LoadFromXml(
            self, FindElementByTagName(colladaNode, DaeSyntax.LIBRARY_GEOMETRIES)
        )
        self.lightsLibrary.LoadFromXml(
            self, FindElementByTagName(colladaNode, DaeSyntax.LIBRARY_LIGHTS)
        )
        self.materialsLibrary.LoadFromXml(
            self, FindElementByTagName(colladaNode, DaeSyntax.LIBRARY_MATERIALS)
        )
        self.nodesLibrary.LoadFromXml(
            self, FindElementByTagName(colladaNode, DaeSyntax.LIBRARY_NODES)
        )
        self.visualScenesLibrary.LoadFromXml(
            self, FindElementByTagName(colladaNode, DaeSyntax.LIBRARY_VISUAL_SCENES)
        )

        self.physicsMaterialsLibrary.LoadFromXml(
            self, FindElementByTagName(colladaNode, DaeSyntax.LIBRARY_PHYSICS_MATERIALS)
        )
        self.physicsModelsLibrary.LoadFromXml(
            self, FindElementByTagName(colladaNode, DaeSyntax.LIBRARY_PHYSICS_MODELS)
        )
        self.physicsScenesLibrary.LoadFromXml(
            self, FindElementByTagName(colladaNode, DaeSyntax.LIBRARY_PHYSICS_SCENES)
        )
    ...
    def SaveDocumentToFile(self, filename):
        ...
        with open(filename, "w", encoding="utf-8") as fileref:
    ...
    def __str__(self):
        return "%s version: %s, xmlns: %s, asset: %s, extras: %s, scene: %s" % (
            type(self),
            self.version,
            self.xmlns,
            self.asset,
...
class DaeElement(DaeEntity):
    ...
    def __str__(self):
        return super(DaeElement, self).__str__() + "id: %s, name: %s" % (
            self.id,
            self.name,
        )
...
class DaeLibrary(DaeElement):
    ...
    def LoadFromXml(self, daeDocument, xmlNode):
        ...
        self.extras = CreateObjectsFromXml(
            daeDocument, xmlNode, DaeSyntax.EXTRA, DaeExtra
        )
        self.asset = CreateObjectFromXml(
            daeDocument, xmlNode, DaeSyntax.ASSET, DaeAsset
        )
        self.items = CreateObjectsFromXml(
            daeDocument, xmlNode, self.__objectSyntax, self.__objectType
        )
    ...
    def __str__(self):
        return super(
            DaeLibrary, self
        ).__str__() + "extras: %s, asset: %s, items: %s" % (
            self.extras,
            self.asset,
            self.items,
        )
...
class DaeAsset(DaeEntity):
    ...
    def LoadFromXml(self, daeDocument, xmlNode):
        ...
        self.contributors = CreateObjectsFromXml(
            daeDocument, xmlNode, DaeSyntax.CONTRIBUTOR, DaeContributor
        )
        # Get created
        ...
        # Get keywords
        self.keywords = GetStringArrayFromNodes(
            xmlNode.getElementsByTagName(DaeSyntax.KEYWORDS)
        )
        # Get Unit
        self.unit.LoadFromXml(
            daeDocument, FindElementByTagName(xmlNode, DaeSyntax.UNIT)
        )
        # Get upAxis
    ...
    def __str__(self):
        return super(
            DaeAsset, self
        ).__str__() + "contributors: %s, created: %s, modified: %s, revision: %s, title: %s, subject: %s, keywords: %s, unit: %s, upAxis: %s" % (
            self.contributors,
            self.created,
            self.modified,
            self.revision,
...
class DaeScene(DaeEntity):
    ...
    def LoadFromXml(self, daeDocument, xmlNode):
        if xmlNode is None:
            return
        self.iVisualScenes = CreateObjectsFromXml(
            daeDocument,
            xmlNode,
            DaeSyntax.INSTANCE_VISUAL_SCENE,
            DaeVisualSceneInstance,
        )
        self.iPhysicsScenes = CreateObjectsFromXml(
            daeDocument,
            xmlNode,
            DaeSyntax.INSTANCE_PHYSICS_SCENE,
    ...
    def __str__(self):
        return super(
            DaeScene, self
        ).__str__() + "extras: %s, visualScenes: %s, physicsScenes: %s" % (
            self.extras,
            self.iVisualScenes,
            self.iPhysicsScenes,
        )
...
class DaeUnit(DaeEntity):
    ...
    def __str__(self):
        return super(DaeUnit, self).__str__() + " name: %s, meter: %s" % (
            self.name,
            self.meter,
        )
...
class DaeContributor(DaeEntity):
    ...
    def LoadFromXml(self, daeDocument, xmlNode):
        ...
        self.authoringTool = ReadContents(
            FindElementByTagName(xmlNode, DaeSyntax.AUTHORING_TOOL)
        )
        self.comments = ReadContents(FindElementByTagName(xmlNode, DaeSyntax.COMMENTS))
        self.copyright = ReadContents(
            FindElementByTagName(xmlNode, DaeSyntax.COPYRIGHT)
        )
        self.sourceData = ReadContents(
            FindElementByTagName(xmlNode, DaeSyntax.SOURCE_DATA)
        )
    ...
    def __str__(self):
        return super(
            DaeContributor, self
        ).__str__() + "author: %s, authoring_tool: %s, comments: %s, copyright: %s, sourceData: %s" % (
            self.author,
            self.authoringTool,
            self.comments,
            self.copyright,
...
class DaeAnimation(DaeElement):
    ...
    def LoadFromXml(self, daeDocument, xmlNode):
        ...
        self.channels = CreateObjectsFromXml(
            daeDocument, xmlNode, DaeSyntax.CHANNEL, DaeChannel
        )
        self.samplers = CreateObjectsFromXml(
            daeDocument, xmlNode, DaeSyntax.SAMPLER, DaeSampler
        )
        self.sources = CreateObjectsFromXml(
...
class DaeSampler(DaeEntity):
    ...
    def LoadFromXml(self, daeDocument, xmlNode):
        ...
        self.inputs = CreateObjectsFromXml(
            daeDocument, xmlNode, DaeSyntax.INPUT, DaeInput
        )
...
class DaeCamera(DaeElement):
    ...
    def LoadFromXml(self, daeDocument, xmlNode):
        ...
        self.optics.LoadFromXml(
            daeDocument, FindElementByTagName(xmlNode, DaeSyntax.OPTICS)
        )
        self.imager = CreateObjectFromXml(
            daeDocument, xmlNode, DaeSyntax.IMAGER, DaeImager
        )
    ...
    def __str__(self):
        return super(
            DaeCamera, self
        ).__str__() + "asset: %s, optics: %s, imager: %s, extras: %s" % (
            self.asset,
            self.optics,
            self.imager,
            self.extras,
...
class DaeGeometry(DaeElement):
    ...
    def LoadFromXml(self, daeDocument, xmlNode):
        ...
        if self.data is None:
            self.data = CreateObjectFromXml(
                daeDocument, xmlNode, DaeSyntax.CONVEX_MESH, DaeConvexMesh
            )
        if self.data is None:
            self.data = CreateObjectFromXml(
                daeDocument, xmlNode, DaeSyntax.SPLINE, DaeSpline
            )
    ...
    def __str__(self):
        return super(
            DaeGeometry, self
        ).__str__() + " assets: %s, data: %s, extras: %s" % (
            self.asset,
            self.data,
            self.extras,
        )
...
class DaeMesh(DaeEntity):
    ...
    def LoadFromXml(self, daeDocument, xmlNode):
        if xmlNode is None:
            return
        self.vertices = CreateObjectFromXml(
            daeDocument, xmlNode, DaeSyntax.VERTICES, DaeVertices
        )
        self.sources = CreateObjectsFromXml(
            daeDocument, xmlNode, DaeSyntax.SOURCE, DaeSource
        )
        ...
        linestrips = CreateObjectsFromXml(
            daeDocument, xmlNode, DaeSyntax.LINESTRIPS, DaeLineStrips
        )
        polygons = CreateObjectsFromXml(
            daeDocument, xmlNode, DaeSyntax.POLYGONS, DaePolygons
        )
        polylist = CreateObjectsFromXml(
            daeDocument, xmlNode, DaeSyntax.POLYLIST, DaePolylist
        )
        triangles = CreateObjectsFromXml(
            daeDocument, xmlNode, DaeSyntax.TRIANGLES, DaeTriangles
        )
        trifans = CreateObjectsFromXml(
            daeDocument, xmlNode, DaeSyntax.TRIFANS, DaeTriFans
        )
        tristrips = CreateObjectsFromXml(
            daeDocument, xmlNode, DaeSyntax.TRISTRIPS, DaeTriStrips
        )
        if lines is not None:
            self.primitives += lines
        if linestrips is not None:
            self.primitives += linestrips
        if polygons is not None:
            self.primitives += polygons
        if polylist is not None:
            self.primitives += polylist
        if triangles is not None:
            self.primitives += triangles
        if trifans is not None:
            self.primitives += trifans
        if tristrips is not None:
            self.primitives += tristrips
...
class DaeVertices(DaeElement):
    ...
    def __str__(self):
        return super(DaeVertices, self).__str__() + " inputs: %s, extras: %s" % (
            self.inputs,
            self.extras,
        )
...
class DaeSource(DaeElement):
    ...
    def LoadFromXml(self, daeDocument, xmlNode):
        ...
        if xmlNode is None:
            return
        bools = CreateObjectFromXml(
            daeDocument, xmlNode, DaeSyntax.BOOL_ARRAY, DaeBoolArray
        )
        floats = CreateObjectFromXml(
            daeDocument, xmlNode, DaeSyntax.FLOAT_ARRAY, DaeFloatArray
        )
        ints = CreateObjectFromXml(
            daeDocument, xmlNode, DaeSyntax.INT_ARRAY, DaeIntArray
        )
        names = CreateObjectFromXml(
            daeDocument, xmlNode, DaeSyntax.NAME_ARRAY, DaeNameArray
        )
        ...
        self.techniqueCommon = CreateObjectFromXml(
            daeDocument,
            xmlNode,
            DaeSyntax.TECHNIQUE_COMMON,
            DaeSource.DaeTechniqueCommon,
        )
        self.techniques = CreateObjectsFromXml(
            daeDocument, xmlNode, DaeSyntax.TECHNIQUE, DaeTechnique
        )
        if not (self.techniqueCommon is None):
            for i in range(0, self.techniqueCommon.accessor.count):
                ...
                for j in range(0, self.techniqueCommon.accessor.stride):
                    vec.append(
                        self.source.data[i * self.techniqueCommon.accessor.stride + j]
                    )
    ...
    def __str__(self):
        return super(
            DaeSource, self
        ).__str__() + " source: %s, techniqueCommon: %s, techniques: %s" % (
            self.source,
            self.techniqueCommon,
            self.techniques,
        )
    ...
    class DaeTechniqueCommon(DaeEntity):
        ...
        def LoadFromXml(self, daeDocument, xmlNode):
            self.accessor = CreateObjectFromXml(
                daeDocument, xmlNode, DaeSyntax.ACCESSOR, DaeAccessor
            )
        ...
        def __str__(self):
            return super(
                DaeSource.DaeTechniqueCommon, self
            ).__str__() + " accessor: %s" % (self.accessor)
...
class DaeLight(DaeElement):
    ...
    def LoadFromXml(self, daeDocument, xmlNode):
        ...
        lightSourceNode = RemoveWhiteSpaceNode(
            FindElementByTagName(xmlNode, DaeSyntax.TECHNIQUE_COMMON)
        ).firstChild
    ...
    def __str__(self):
        return super(
            DaeLight, self
        ).__str__() + " techniqueCommon: %s, techniques: %s" % (
            self.techniqueCommon,
            self.techniques,
        )
    ...
    class DaeTechniqueCommon(DaeEntity):
        ...
        def LoadFromXml(self, daeDocument, xmlNode):
            self.color = ToFloat3(
                ReadContents(FindElementByTagName(xmlNode, DaeSyntax.COLOR))
            )
        ...
        def __str__(self):
            return super(DaeLight.DaeTechniqueCommon, self).__str__() + " color: %s" % (
                self.color
            )
    ...
    class DaeSpot(DaeTechniqueCommon):
        ...
        def LoadFromXml(self, daeDocument, xmlNode):
            ...
            self.constantAttenuation = CastFromXml(
                daeDocument,
                xmlNode,
                DaeSyntax.CONSTANT_ATTENUATION,
                float,
                self.defConstantAttenuation,
                1.0,
            )
            self.linearAttenuation = CastFromXml(
                daeDocument,
                xmlNode,
                DaeSyntax.LINEAR_ATTENUATION,
                float,
                self.defLinearAttenuation,
                0,
            )
            self.quadraticAttenuation = CastFromXml(
                daeDocument,
                xmlNode,
                DaeSyntax.QUADRATIC_ATTENUATION,
                float,
                self.defQuadraticAttenuation,
                0,
            )
            self.falloffAngle = CastFromXml(
                daeDocument,
                xmlNode,
                DaeSyntax.FALLOFF_ANGLE,
                float,
                self.defFalloffAngle,
                180.0,
            )
            self.falloffExponent = CastFromXml(
                daeDocument,
                xmlNode,
                DaeSyntax.FALLOFF_EXPONENT,
                float,
                self.defFalloffExponent,
                0,
            )
        ...
        def __str__(self):
            return super(
                DaeLight.DaeSpot, self
            ).__str__() + " const.att: %s, lin.att: %s, quad.att: %s, falloffAngle: %s, falloffExponent: %s" % (
                self.constantAttenuation,
                self.linearAttenuation,
                self.quadraticAttenuation,
                self.falloffAngle,
    ...
    class DaePoint(DaeTechniqueCommon):
        ...
        def LoadFromXml(self, daeDocument, xmlNode):
            ...
            self.constantAttenuation = CastFromXml(
                daeDocument, xmlNode, DaeSyntax.CONSTANT_ATTENUATION, float, 1
            )
            self.linearAttenuation = CastFromXml(
                daeDocument, xmlNode, DaeSyntax.LINEAR_ATTENUATION, float, 0
            )
            self.quadraticAttenuation = CastFromXml(
                daeDocument, xmlNode, DaeSyntax.QUADRATIC_ATTENUATION, float, 0
            )
        ...
        def SaveToXml(self, daeDocument):
            ...
            AppendTextChild(
                node.firstChild, DaeSyntax.LINEAR_ATTENUATION, self.linearAttenuation
            )
            AppendTextChild(
                node.firstChild,
                DaeSyntax.QUADRATIC_ATTENUATION,
        ...
        def __str__(self):
            return super(
                DaeLight.DaePoint, self
            ).__str__() + " const.att: %s, lin.att: %s, quad.att: %s" % (
                self.constantAttenuation,
                self.linearAttenuation,
                self.quadraticAttenuation,
            )
...
class DaeVisualScene(DaeElement):
    ...
    def __str__(self):
        return super(
            DaeVisualScene, self
        ).__str__() + " asset: %s, nodes: %s, extras: %s" % (
            self.asset,
            self.nodes,
            self.extras,
        )
...
class DaeNode(DaeElement):
    ...
    def LoadFromXml(self, daeDocument, xmlNode):
        ...
        typeVal = ReadAttribute(xmlNode, DaeSyntax.TYPE)
        if typeVal == DaeSyntax.TYPE_JOINT:
        ...
        # Get the instances
        self.iAnimations = CreateObjectsFromXml(
            daeDocument, xmlNode, DaeSyntax.INSTANCE_ANIMATION, DaeAnimationInstance
        )
        self.iCameras = CreateObjectsFromXml(
            daeDocument, xmlNode, DaeSyntax.INSTANCE_CAMERA, DaeCameraInstance
        )
        self.iControllers = CreateObjectsFromXml(
            daeDocument, xmlNode, DaeSyntax.INSTANCE_CONTROLLER, DaeControllerInstance
        )
        self.iGeometries = CreateObjectsFromXml(
            daeDocument, xmlNode, DaeSyntax.INSTANCE_GEOMETRY, DaeGeometryInstance
        )
        self.iLights = CreateObjectsFromXml(
            daeDocument, xmlNode, DaeSyntax.INSTANCE_LIGHT, DaeLightInstance
        )
        self.iNodes = CreateObjectsFromXml(
            daeDocument, xmlNode, DaeSyntax.INSTANCE_NODE, DaeNodeInstance
        )
        self.iVisualScenes = CreateObjectsFromXml(
            daeDocument,
            xmlNode,
    ...
    def SaveToXml(self, daeDocument):
        ...
        # BUGFIX: comparar com o enum numérico, não com string.
        if self.type == DaeNode.JOINT:
        ...
        for i in self.transforms:
            ...
            if i[0] == DaeSyntax.MATRIX:
                ...
                if i[0] == DaeSyntax.SCALE:
                    SetAttribute(
                        AppendTextChild(node, i[0], val, None),
                        DaeSyntax.SID,
                        DaeSyntax.SCALE,
                    )
...
class DaeOptics(DaeEntity):
    ...
    def LoadFromXml(self, daeDocument, xmlNode):
        ...
        opticsSourceNode = RemoveWhiteSpaceNode(
            FindElementByTagName(xmlNode, DaeSyntax.TECHNIQUE_COMMON)
        ).firstChild
    ...
    def __str__(self):
        return super(
            DaeOptics, self
        ).__str__() + " extras: %s, techniqueCommon: %s, techniques: %s" % (
            self.extras,
            self.techniqueCommon,
            self.techniques,
        )
    ...
    class DaeTechniqueCommon(DaeEntity):
        ...
        def LoadFromXml(self, daeDocument, xmlNode):
            self.znear = ToFloat(
                ReadContents(FindElementByTagName(xmlNode, DaeSyntax.ZNEAR))
            )
            self.zfar = ToFloat(
                ReadContents(FindElementByTagName(xmlNode, DaeSyntax.ZFAR))
            )
            self.aspectRatio = ToFloat(
                ReadContents(FindElementByTagName(xmlNode, DaeSyntax.ASPECT_RATIO))
            )
        ...
        def __str__(self):
            return super(
                DaeOptics.DaeTechniqueCommon, self
            ).__str__() + " znear: %s, zfar: %s, aspectRatio: %s" % (
                self.znear,
                self.zfar,
                self.aspectRatio,
            )
    ...
    class DaePerspective(DaeTechniqueCommon):
        ...
        def LoadFromXml(self, daeDocument, xmlNode):
            ...
            self.xfov = ToFloat(
                ReadContents(FindElementByTagName(xmlNode, DaeSyntax.XFOV))
            )
            self.yfov = ToFloat(
                ReadContents(FindElementByTagName(xmlNode, DaeSyntax.YFOV))
            )
        ...
        def SaveToXml(self, daeDocument):
            ...
            super(DaeOptics.DaePerspective, self).SavePropertiesToXml(
                daeDocument, node.firstChild
            )
            return node

        def __str__(self):
            return super(
                DaeOptics.DaePerspective, self
            ).__str__() + " xfov: %s, yfov: %s" % (self.xfov, self.yfov)
    ...
    class DaeOrthoGraphic(DaeTechniqueCommon):
        ...
        def LoadFromXml(self, daeDocument, xmlNode):
            ...
            self.xmag = ToFloat(
                ReadContents(FindElementByTagName(xmlNode, DaeSyntax.XMAG))
            )
            self.ymag = ToFloat(
                ReadContents(FindElementByTagName(xmlNode, DaeSyntax.YMAG))
            )
        ...
        def __str__(self):
            return super(
                DaeOptics.DaeOrthoGraphic, self
            ).__str__() + "xmag: %s, ymag: %s" % (self.xmag, self.ymag)
...
class DaeImager(DaeEntity):
    ...
    def SaveToXml(self, daeDocument):
        # BUGFIX: super() estava errado aqui no código legado.
        node = super(DaeImager, self).SaveToXml(daeDocument)
        # Add the techniques
        AppendChilds(daeDocument, node, self.techniques)
        # Add the extra's
        AppendChilds(self, node, self.extras)
    ...
    def __str__(self):
        return super(DaeImager, self).__str__() + " extras: %s, techniques: %s" % (
            self.extras,
            self.techniques,
        )
...
class DaeAccessor(DaeEntity):
    ...
    def LoadFromXml(self, daeDocument, xmlNode):
        ...
        self.params = CreateObjectsFromXml(
            daeDocument, xmlNode, DaeSyntax.PARAM, DaeParam
        )
...
class DaeTriangles(DaePrimitive):
    ...
    def LoadFromXml(self, daeDocument, xmlNode):
        ...
        self.triangles = ToIntList(
            ReadContents(FindElementByTagName(xmlNode, DaeSyntax.P))
        )
        self.inputs = CreateObjectsFromXml(
            daeDocument, xmlNode, DaeSyntax.INPUT, DaeInput
        )
...
class DaeControllerInstance(DaeInstance):
    ...
    def LoadFromXml(self, daeDocument, xmlNode):
        ...
        self.skeletons = CreateObjectsFromXml(
            daeDocument, xmlNode, DaeSyntax.SKELETON, DaeSkeleton
        )
        self.bindMaterials = CreateObjectsFromXml(
            daeDocument, xmlNode, DaeFxSyntax.BIND_MATERIAL, DaeFxBindMaterial
        )
...
class DaeFxBindMaterial(DaeEntity):
    ...
    def LoadFromXml(self, daeDocument, xmlNode):
        self.techniqueCommon = CreateObjectFromXml(
            daeDocument,
            xmlNode,
            DaeFxSyntax.TECHNIQUE_COMMON,
            DaeFxBindMaterial.DaeFxTechniqueCommon,
        )
    ...
    class DaeFxTechniqueCommon(DaeEntity):
        ...
        def LoadFromXml(self, daeDocument, xmlNode):
            self.iMaterials = CreateObjectsFromXml(
                daeDocument,
                xmlNode,
                DaeFxSyntax.INSTANCE_MATERIAL,
                DaeFxMaterialInstance,
            )

        def SaveToXml(self, daeDocument):
            node = super(DaeFxBindMaterial.DaeFxTechniqueCommon, self).SaveToXml(
                daeDocument
            )
...
class DaeFxMaterialInstance(DaeEntity):
    ...
    def __str__(self):
        return super(
            DaeFxMaterialInstance, self
        ).__str__() + " target: %s, symbol: %s" % (self.target, self.symbol)
...
class DaeFxMaterial(DaeElement):
    ...
    def LoadFromXml(self, daeDocument, xmlNode):
        ...
        self.iEffects = CreateObjectsFromXml(
            daeDocument, xmlNode, DaeFxSyntax.INSTANCE_EFFECT, DaeFxEffectInstance
        )
    ...
    def __str__(self):
        return super(
            DaeFxMaterial, self
        ).__str__() + " assets: %s, iEffects: %s, extras: %s" % (
            self.asset,
            self.iEffects,
            self.extras,
        )
...
class DaeFxEffect(DaeElement):
    ...
    def LoadFromXml(self, daeDocument, xmlNode):
        ...
        self.profileCommon = CreateObjectFromXml(
            daeDocument, xmlNode, DaeFxSyntax.PROFILE_COMMON, DaeFxProfileCommon
        )
    ...
    def __str__(self):
        return super(DaeFxEffect, self).__str__() + ", profileCommon: %s" % (
            self.profileCommon
        )
...
class DaeFxProfileCommon(DaeEntity):
    ...
    def LoadFromXml(self, daeDocument, xmlNode):
        self.images = CreateObjectsFromXml(
            daeDocument, xmlNode, DaeFxSyntax.IMAGE, DaeFxImage
        )
        self.newParams = CreateObjectsFromXml(
            daeDocument, xmlNode, DaeFxSyntax.NEWPARAM, DaeFxNewParam
        )
        self.technique = CreateObjectFromXml(
            daeDocument, xmlNode, DaeFxSyntax.TECHNIQUE, DaeFxTechnique
        )
    ...
    def __str__(self):
        return super(
            DaeFxProfileCommon, self
        ).__str__() + ", technique: %s, images: %s, newParams: %s" % (
            self.technique,
            self.images,
            self.newParams,
        )
...
class DaeFxImage(DaeEntity):
    ...
    def LoadFromXml(self, daeDocument, xmlNode):
        super().LoadFromXml(daeDocument, xmlNode)

    def SaveToXml(self, daeDocument):
        node = super().SaveToXml(daeDocument)
        return node
...
class DaeFxTechnique(DaeEntity):
    ...
    def LoadFromXml(self, daeDocument, xmlNode):
        ...
        # Escolhe shader: constant / lambert / blinn / phong
        if FindElementByTagName(xmlNode, DaeFxSyntax.CONSTANT):
            self.shader = CreateObjectFromXml(
                daeDocument, xmlNode, DaeFxSyntax.CONSTANT, DaeFxShadeConstant
            )
        elif FindElementByTagName(xmlNode, DaeFxSyntax.LAMBERT):
            self.shader = CreateObjectFromXml(
                daeDocument, xmlNode, DaeFxSyntax.LAMBERT, DaeFxShadeLambert
            )
        elif FindElementByTagName(xmlNode, DaeFxSyntax.BLINN):
            self.shader = CreateObjectFromXml(
                daeDocument, xmlNode, DaeFxSyntax.BLINN, DaeFxShadeBlinn
            )
        elif FindElementByTagName(xmlNode, DaeFxSyntax.PHONG):
            self.shader = CreateObjectFromXml(
                daeDocument, xmlNode, DaeFxSyntax.PHONG, DaeFxShadePhong
            )
        else:
            # fallback seguro (evita shader None)
            self.shader = DaeFxShadeConstant()
    ...
    def __str__(self):
        return super().__str__() + " shader: %s" % (self.shader)
...
class DaeFxShadeConstant(DaeEntity):
    ...
    def LoadFromXml(self, daeDocument, xmlNode):
        self.emission = CreateObjectFromXml(
            daeDocument,
            xmlNode,
            DaeFxSyntax.EMISSION,
            DaeFxCommonColorAndTextureContainer,
            True,
        )
        self.reflective = CreateObjectFromXml(
            daeDocument,
            xmlNode,
            DaeFxSyntax.REFLECTIVE,
            DaeFxCommonColorAndTextureContainer,
            True,
        )
        self.reflectivity = CreateObjectFromXml(
            daeDocument,
            xmlNode,
            DaeFxSyntax.REFLECTIVITY,
            DaeFxCommonFloatAndParamContainer,
            True,
        )
        self.transparent = CreateObjectFromXml(
            daeDocument,
            xmlNode,
            DaeFxSyntax.TRANSPARENT,
            DaeFxCommonColorAndTextureContainer,
            True,
        )
        self.transparency = CreateObjectFromXml(
            daeDocument,
            xmlNode,
            DaeFxSyntax.TRANSPARENCY,
            DaeFxCommonFloatAndParamContainer,
            True,
        )
        self.indexOfRefraction = CreateObjectFromXml(
            daeDocument,
            xmlNode,
            DaeFxSyntax.INDEXOFREFRACTION,
            DaeFxCommonFloatAndParamContainer,
            True,
        )
    ...
    def SaveToXml(self, daeDocument):
        ...
        # Lambert/Blinn/Phong herdam daqui, então este isinstance funciona
        if isinstance(self, DaeFxShadeLambert):
            AppendChild(daeDocument, node, getattr(self, "ambient", None))
            AppendChild(daeDocument, node, getattr(self, "diffuse", None))

            if isinstance(self, DaeFxShadeBlinn):
                AppendChild(daeDocument, node, getattr(self, "specular", None))
                AppendChild(daeDocument, node, getattr(self, "shininess", None))
...
class DaeFxShadeLambert(DaeFxShadeConstant):
    ...
    def LoadFromXml(self, daeDocument, xmlNode):
        super().LoadFromXml(daeDocument, xmlNode)
        self.ambient = CreateObjectFromXml(
            daeDocument,
            xmlNode,
            DaeFxSyntax.AMBIENT,
            DaeFxCommonColorAndTextureContainer,
        ...
        self.diffuse = CreateObjectFromXml(
            daeDocument,
            xmlNode,
            DaeFxSyntax.DIFFUSE,
            DaeFxCommonColorAndTextureContainer,
    ...
    def SaveToXml(self, daeDocument):
        # O super já salva ambient/diffuse (via isinstance)
        node = super().SaveToXml(daeDocument)
        return node
    ...
    def AddValue(self, type, val):
        ...
        if type == DaeFxSyntax.DIFFUSE:
            ...
            if isinstance(val, DaeFxTexture):  # texture
...
class DaeFxShadeBlinn(DaeFxShadeLambert):
    ...
    def LoadFromXml(self, daeDocument, xmlNode):
        super().LoadFromXml(daeDocument, xmlNode)
        self.specular = CreateObjectFromXml(
            daeDocument,
            xmlNode,
            DaeFxSyntax.SPECULAR,
            DaeFxCommonColorAndTextureContainer,
            True,
        )
        self.shininess = CreateObjectFromXml(
            daeDocument,
            xmlNode,
            DaeFxSyntax.SHININESS,
            DaeFxCommonFloatAndParamContainer,
            True,
        )

    def SaveToXml(self, daeDocument):
        # O super já salva specular/shininess (via isinstance)
        node = super().SaveToXml(daeDocument)
        return node
...
class DaeFxCommonColorAndTextureContainer(DaeEntity):
    ...
    def LoadFromXml(self, daeDocument, xmlNode):
        self.color = CreateObjectFromXml(
            daeDocument, xmlNode, DaeFxSyntax.COLOR, DaeFxColor
        )
        self.texture = CreateObjectFromXml(
            daeDocument, xmlNode, DaeFxSyntax.TEXTURE, DaeFxTexture
        )
...
class DaeFxTexture(DaeEntity):
    def __init__(self):
        super().__init__()
        # Pode virar string (sid) ou objeto (dependendo do seu loader/lib)
        self.texture = ""
        self.textCoord = ""
        self.syntax = DaeFxSyntax.TEXTURE

    def LoadFromXml(self, daeDocument, xmlNode):
        tex = ReadAttribute(xmlNode, DaeFxSyntax.TEXTURE)
        # Nem todo COLLADA liga texture->image direto; então seja tolerante:
        found = None
        try:
            found = daeDocument.imagesLibrary.FindObject(tex)
        except Exception:
            found = None
        self.texture = found if found is not None else tex
        self.textCoord = ReadAttribute(xmlNode, DaeFxSyntax.TEXCOORD)

    def SaveToXml(self, daeDocument):
        node = super().SaveToXml(daeDocument)

        # Se for objeto, tenta tirar um identificador útil; se for string, usa direto
        tex_value = self.texture
        if tex_value is None:
            tex_value = ""
        elif not isinstance(tex_value, str):
            tex_value = (
                getattr(tex_value, "id", None)
                or getattr(tex_value, "sid", None)
                or str(tex_value)
            )

        SetAttribute(node, DaeFxSyntax.TEXTURE, StripString(tex_value))
...
class DaePhysicsScene(DaeElement):
    ...
    def LoadFromXml(self, daeDocument, xmlNode):
        ...
        self.iPhysicsModels = CreateObjectsFromXml(
            daeDocument,
            xmlNode,
            DaePhysicsSyntax.INSTANCE_PHYSICS_MODEL,
    ...
    def SaveToXml(self, daeDocument):
        ...
        AppendChilds(
            daeDocument, node, self.extras
        )  # <-- BUG corrigido (antes era self)
        return node

    def __str__(self):
        return super().__str__() + " asset: %s, iPhysicsModels: %s, extras: %s" % (
            self.asset,
            self.iPhysicsModels,
            self.extras,
        )
...
class DaePhysicsModelInstance(DaeInstance):
    ...
    def LoadFromXml(self, daeDocument, xmlNode):
        ...
        self.iRigidBodies = self.CreateInstanceRigidBodies(
            daeDocument, xmlNode, self.object
        )
    ...
    def CreateInstanceRigidBodies(self, daeDocument, xmlNode, physicsModel):
        ...
        for node in nodes:
            obj = DaeRigidBodyInstance()
            obj.LoadFromXml(daeDocument, node)
            obj.body = (
                physicsModel.FindRigidBody(obj.bodyString)
                if physicsModel is not None
                else None
            )

            n = None
            for visualScene in daeDocument.visualScenesLibrary.items:
                n = visualScene.FindNode(obj.targetString)
                if n is not None:
                    break
...
class DaeRigidBodyInstance(DaeEntity):
    ...
    def LoadFromXml(self, daeDocument, xmlNode):
        ...
        tgt = ReadAttribute(xmlNode, DaeSyntax.TARGET)
        self.targetString = tgt[1:] if tgt and tgt.startswith("#") else (tgt or "")
...
class DaePhysicsModel(DaeElement):
    ...
    def LoadFromXml(self, daeDocument, xmlNode):
        super().LoadFromXml(daeDocument, xmlNode)
        self.rigidBodies = CreateObjectsFromXml(
            daeDocument, xmlNode, DaePhysicsSyntax.RIGID_BODY, DaeRigidBody
        )
...
class DaePhysicsMaterial(DaeElement):
    ...
    def LoadFromXml(self, daeDocument, xmlNode):
        ...
        self.techniqueCommon = CreateObjectFromXml(
            ...
            xmlNode,
            DaeSyntax.TECHNIQUE_COMMON,
            DaePhysicsMaterial.DaeTechniqueCommon,
        )
    ...
    class DaeTechniqueCommon(DaeEntity):
        ...
        def LoadFromXml(self, daeDocument, xmlNode):
            self.dynamicFriction = CastFromXml(
                daeDocument, xmlNode, DaePhysicsSyntax.DYNAMIC_FRICTION, float, 0
            )
            self.restitution = CastFromXml(
                daeDocument, xmlNode, DaePhysicsSyntax.RESTITUTION, float, 0
            )
            self.staticFriction = CastFromXml(
                daeDocument, xmlNode, DaePhysicsSyntax.STATIC_FRICTION, float, 0
            )

        def SaveToXml(self, daeDocument):
            node = super().SaveToXml(daeDocument)
            AppendTextChild(
                node, DaePhysicsSyntax.DYNAMIC_FRICTION, self.dynamicFriction, None
            )
            AppendTextChild(node, DaePhysicsSyntax.RESTITUTION, self.restitution, None)
            AppendTextChild(
                node, DaePhysicsSyntax.STATIC_FRICTION, self.staticFriction, None
            )
            return node
...
class DaeRigidBody(DaeEntity):
    ...
    def LoadFromXml(self, daeDocument, xmlNode):
        ...
        self.techniqueCommon = CreateObjectFromXml(
            ...
            xmlNode,
            DaeSyntax.TECHNIQUE_COMMON,
            DaeRigidBody.DaeTechniqueCommon,
        )
    ...
    class DaeTechniqueCommon(DaeEntity):
        ...
        def LoadFromXml(self, daeDocument, xmlNode):
            self.iPhysicsMaterial = CreateObjectFromXml(
                daeDocument,
                xmlNode,
                DaePhysicsSyntax.INSTANCE_PHYSICS_MATERIAL,
                DaePhysicsMaterialInstance,
            )
            self.physicsMaterial = CreateObjectFromXml(
                daeDocument,
                xmlNode,
                DaePhysicsSyntax.PHYSICS_MATERIAL,
                DaePhysicsMaterial,
            )
            self.dynamic = CastFromXml(
                daeDocument, xmlNode, DaePhysicsSyntax.DYNAMIC, bool, True
            )
            self.mass = CastFromXml(
                daeDocument, xmlNode, DaePhysicsSyntax.MASS, float, 1
            )

            inertia_node = FindElementByTagName(xmlNode, DaePhysicsSyntax.INERTIA)
            self.inertia = (
                ToFloat3(ReadContents(inertia_node))
            ...
            for shapeNode in shapeNodes:
                ...
                if s is not None:
                    ...
                    if s is not None:
                        ...
                        if s is not None:
                            ...
                            s = FindElementByTagName(
                                shapeNode, DaeSyntax.INSTANCE_GEOMETRY
                            )
                            if s is not None:
                                b = DaeGeometryShape()
                            else:
                                s = FindElementByTagName(
                                    shapeNode, DaePhysicsSyntax.CYLINDER
                                )
                                if s is not None:
                                    b = DaeCylinderShape()
                                else:
                                    s = FindElementByTagName(
                                        shapeNode, DaePhysicsSyntax.TAPERED_CYLINDER
                                    )
                                    if s is not None:
                                        b = DaeTaperedCylinderShape()
                                    else:
                                        s = FindElementByTagName(
                                            shapeNode, DaePhysicsSyntax.CAPSULE
                                        )
                                        if s is not None:
                                            b = (
                                                DaeCapsuleShape()
                                            )  # <-- BUG corrigido (antes era DaeCapsule())
                                        else:
                                            s = FindElementByTagName(
                                                shapeNode,
                                                DaePhysicsSyntax.TAPERED_CAPSULE,
                                            )
                                            if s is not None:
                                                # Se essa classe existir no seu projeto, beleza. Se não, evita crash.
                                                b = (
                                                    DaeTaperedCapsuleShape()
                                                    if "DaeTaperedCapsuleShape"
                                                    in globals()
                                                    else None
                                                )
                ...
                if b is not None and s is not None:
                    ...
                    Debug.Debug(
                        "RigidBody technique_common: shape desconhecido/nao suportado",
                        "WARN",
                    )
        ...
        def GetPhysicsMaterial(self):
            if self.physicsMaterial is not None:
                return self.physicsMaterial
            return (
                self.iPhysicsMaterial.object
                if self.iPhysicsMaterial is not None
                else None
            )
...
class DaeShape(DaeEntity):
    ...
    def LoadFromXml(self, daeDocument, xmlNode):
        self.iGeometry = CreateObjectFromXml(
            daeDocument, xmlNode, DaeSyntax.INSTANCE_GEOMETRY, DaeGeometryInstance
        )
        self.mass = CastFromXml(
            daeDocument, xmlNode, DaePhysicsSyntax.MASS, float, None
        )
        self.density = CastFromXml(
            daeDocument, xmlNode, DaePhysicsSyntax.DENSITY, float, None
        )
...
class DaeBoxShape(DaeShape):
    ...
    def LoadFromXml(self, daeDocument, xmlNode):
        super().LoadFromXml(daeDocument, xmlNode)
        self.halfExtents = ToFloat3(
            ReadContents(FindElementByTagName(xmlNode, DaePhysicsSyntax.HALF_EXTENTS))
        )
...
class DaeCylinderShape(DaeShape):
    ...
    def LoadFromXml(self, daeDocument, xmlNode):
        super().LoadFromXml(daeDocument, xmlNode)
        self.radius = ToFloat2(
            ReadContents(FindElementByTagName(xmlNode, DaePhysicsSyntax.RADIUS)),
            "Not a valid radius found. Must consist of 2 floats",
        )
...
class DaeTaperedCylinderShape(DaeShape):
    ...
    def LoadFromXml(self, daeDocument, xmlNode):
        super().LoadFromXml(daeDocument, xmlNode)
        self.radius1 = ToFloat2(
            ReadContents(FindElementByTagName(xmlNode, DaePhysicsSyntax.RADIUS1)),
            "Not a valid radius found. Must consist of 2 floats",
        )
        self.radius2 = ToFloat2(
            ReadContents(FindElementByTagName(xmlNode, DaePhysicsSyntax.RADIUS2)),
            "Not a valid radius found. Must consist of 2 floats",
        )
        self.height = CastFromXml(daeDocument, xmlNode, DaeSyntax.HEIGHT, float)
...
class DaePlaneShape(DaeShape):
    ...
    def LoadFromXml(self, daeDocument, xmlNode):
        super().LoadFromXml(daeDocument, xmlNode)
        self.equation = ToFloat4(
            ReadContents(FindElementByTagName(xmlNode, DaePhysicsSyntax.EQUATION))
        )
...
class DaeGeometryShape(DaeShape):
    ...
    def SaveToXml(self, daeDocument):
        return (
            self.iGeometry.SaveToXml(daeDocument)
            if self.iGeometry is not None
            else None
        )
...
def CreateObjectsFromXml(colladaDocument, xmlNode, nodeType, objectType):
    ...
    for node in nodes:
        obj = objectType()
        obj.LoadFromXml(colladaDocument, node)
        objects.append(obj)
    return objects


def CreateObjectFromXml(
    colladaDocument, xmlNode, nodeType, objectType, setSyntax=False
):
    if xmlNode is None:
        return None
    ...
    obj = objectType(nodeType) if setSyntax else objectType()
    obj.LoadFromXml(colladaDocument, node)
    return obj
...
def CastFromXml(colladaDocument, xmlNode, nodeType, cast, default=None):
    ...
    if node is not None:
        ...
        if cast == bool:
            return False if textValue.lower() == "false" else True
...
def AppendTextChild(xmlNode, syntax, obj, default=None):
    if obj is None:
        return
    ...
    return AppendTextInChild(node, obj)


def AppendTextInChild(xmlNode, obj):
    if obj is None:
        return
    text = Text()
    if type(obj) == datetime:
        text.data = obj.isoformat()
    elif type(obj) == list:
        if len(obj) == 0:
            return
        if obj[0] is not None and type(obj[0]) == float:
            obj = RoundList(obj, ROUND)
        text.data = ListToString(obj)
    elif type(obj) == float:
        text.data = round(obj, ROUND)
    elif type(obj) == bool:
        text.data = str(obj).lower()
    else:
        text.data = str(obj)
...
def SetAttribute(xmlNode, syntax, obj):
    if xmlNode is None or obj is None or str(obj) == "":
        return
    xmlNode.setAttribute(syntax, str(obj))
